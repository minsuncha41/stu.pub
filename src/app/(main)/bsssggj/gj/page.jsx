...
import DispatchRequestDialog from "./gjmd";
    ...
    <div className="h-screen min-w-[1280px] overflow-hidden bg-[#0d1930] text-white">
      ...
      <header className="flex h-[64px] items-center border-b border-white/10 bg-[#152646]">
        ...
        <div className="flex items-center !pr-6">
          ...
          <div className="">
            <a href="/info/register" className="!ml-5 flex items-center">
      ...
      <main className="flex h-[calc(100vh-128px)]">
        ...
        {/* Map Section 통합 영역 */}
        <section className="relative flex-1 overflow-hidden bg-[#dce5de]">
          {/* 카카오맵 렌더링 컨테이너 */}
          <div ref={mapRef} className="absolute inset-0 z-0" />

          {mapStatus !== "ready" && (
            <div className="absolute inset-0 z-10 flex items-center justify-center bg-slate-900/80 px-6 text-center text-sm text-slate-100">
              {mapStatus === "missing-key"
                ? "카카오 지도 API 키가 없어 지도를 표시할 수 없습니다. .env.local에 NEXT_PUBLIC_KAKAO_JS_KEY를 추가해 주세요."
                : "지도를 불러오는 중입니다..."}
            </div>
          )}

          {/* 기존 UI 요소 (z-10, z-20 부여하여 지도 위에 표시) */}
          <div className="absolute left-4 top-4 z-10 pointer-events-none">
            <Badge className="!px-3 !py-1.5 bg-red-500 text-white pointer-events-auto">
              ● LIVE
            </Badge>

            <div className="!mt-2 flex rounded-md bg-[#1a2d4f] !p-1 text-xs pointer-events-auto">
          ...
          <div className="absolute left-[42%] top-[45%] z-20 flex h-12 w-12 items-center justify-center rounded-full border-4 border-white bg-red-500 shadow-xl pointer-events-auto">
            <Truck className="h-5 w-5" />
          </div>

          <div className="absolute left-[47%] top-[53%] z-20 rounded-lg bg-[#152646] !px-4 !py-3 shadow-xl pointer-events-auto">
          ...
          <div className="absolute bottom-10 left-4 z-20 w-[220px] rounded-lg bg-[#152646] !p-4 text-xs shadow-xl pointer-events-auto">
          ...
          <div className="absolute right-4 top-1/2 z-20 flex -translate-y-1/2 flex-col rounded-md bg-[#152646] shadow-xl pointer-events-auto">
            <Button variant="ghost" size="icon">
              +
            </Button>
            <Button variant="ghost" size="icon">
